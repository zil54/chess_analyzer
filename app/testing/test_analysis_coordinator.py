...
def test_parse_request_payload_supports_plain_text_and_json() -> None:
    ...
    assert plain_request.display_target_depth == 20
    assert plain_request.worker_target_depth == 26
...
@pytest.mark.asyncio
async def test_handle_websocket_returns_cached_snapshot_when_worker_target_already_met(monkeypatch) -> None:
    ...
    async def fake_get_snapshot(fen: str, target_depth: int | None = None) -> dict:
        return _snapshot(20)

    async def fail_ensure(*args, **kwargs):
        raise AssertionError("ensure_analysis should not be called for a complete cached snapshot")
...
async def test_handle_websocket_starts_worker_and_streams_lagged_db_snapshots(monkeypatch) -> None:
    ...
    async def fake_get_snapshot(fen: str, target_depth: int | None = None):
